<script lang="ts">
    ...
    function setIsBooped(val){isBooped = val;}

    const handleClickEvent = () => {if (parameters.active) alert("CARD CLICKED")};

    const mouseEnterEvent = () => {if (parameters.active) isBooped = true};

    const mouseLeaveEvent = () => {if (parameters.active) isBooped = false};

</script>

<div class={parameters.cardType === 'chapter' ? 'container chapter' : 'container case-study'} style={parameters.active ? "opacity: 100%" : "opacity: 50%"} tabindex="0">
    <div class="preview-image">
        <img class="image" width="766" height="344" src={parameters.previewImage} alt="preview">
    </div>
    <div class="preview-content">
        <div class="circle-button" tabindex="0" on:mouseenter={mouseEnterEvent} on:click={handleClickEvent} on:mouseleave={mouseLeaveEvent}>
